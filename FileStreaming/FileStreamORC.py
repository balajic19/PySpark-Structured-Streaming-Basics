...
spark = SparkSession.builder.master('local[*]').appName('File Streaming ORC').getOrCreate()
...
data_path = os.path.join(os.getcwd(), 'FileStreaming\input_data\orc')
print(data_path)


stream_df = spark.readStream.format('orc').schema(input_parquet_schema).option('header', True)\
    .load(path = data_path)
    # .load(path="C://Users/Balaji/workspace/Python_workspace/PySpark/Intro to Spark Streaming/FileStreaming/input_data/orc")
...
write_df1 = df1.writeStream.format('console').outputMode('complete') \
    ...
    # .option('checkpointLocation', "./FileStreaming/streaming-checkpoint-loc-orc")\
